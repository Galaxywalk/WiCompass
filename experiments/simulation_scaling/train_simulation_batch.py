#!/usr/bin/env python3
"""
Batch training script for SimulationMMBody with different training sets.

All training sets are evaluated against the same benchmark validation set.

Usage:
    # Train all available training sets
    python train_simulation_batch.py
    
    # Train specific training sets
    python train_simulation_batch.py --train_sets k10_q85 k12_q95
...
BENCHMARK_PATH = DATA_ROOT / "benchmark"
...
def train_single_job(train_set: str, base_cfg: dict, train_script: str, 
                     data_root: str, benchmark_path: str, model_scale: str = None):
    """Train a single model with specified training set."""
    import threading
    ...
    job_name = f"sim_{train_set}_{scale_str}"
...
def main():
    parser = argparse.ArgumentParser(description='Batch training for SimulationMMBody')
    ...
    for train_set in train_sets:
        job_name = f"train_{train_set}"
        fut = train_single_job.options(name=job_name).remote(
            train_set, base_cfg, str(TRAIN_SCRIPT),
